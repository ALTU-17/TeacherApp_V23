...
public class HomeworkViewedByActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String hw_desc = intent.getStringExtra("hw_desc");
        String hw_id = intent.getStringExtra("hw_id");
        ...
        getStudentsList(class_id, section_id, academic_yr, hw_id);
        tv_desc.setText(hw_desc);
    ...
    private void getStudentsList(String class_id, String section_id, final String academic_yr, final String notes_id) {
        ...
        params.put("homework_id", notes_id);
        params.put("short_name", name);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(newUrl + "AdminApi/" + "get_students_homework_viewed", new JSONObject(params), new Response.Listener<JSONObject>() {
            @Override
                ...
                try {
                    if (response.getString("status").equals("true")) {
                        ...
                        for (int i = 0; i < result.length(); i++) {
                            ...
                            else if (rollNo.equals("null")|| rollNo.equals("") || rollNo.equals(null)) {
                                rollNo = "";
                                ViewedByListPojo viewedByListPojo = new ViewedByListPojo(rollNo, fName,mName, lName,read_status);
                                viewdByPojoArrayList.add(viewedByListPojo);
                            } else {
                            ...
                            homeworkViewedByAdapter = new HomeworkViewedByAdapter(getApplicationContext(), viewdByPojoArrayList);
                            recyclerview.setAdapter(homeworkViewedByAdapter);
                        }
                    } else {
                        progressBar.setVisibility(View.GONE);
                        Toast.makeText(HomeworkViewedByActivity.this, "No Record", Toast.LENGTH_SHORT).show();
                    }
    ...
    private void getIds() {
        tv_desc = findViewById(R.id.tv_note_desc);
    ...
    public void onBackPressed() {
        ...
        Intent in = new Intent(HomeworkViewedByActivity.this, HomeworkActivity.class);
        startActivity(in);
        in.putExtra("reg_id", reg_id);
        in.putExtra("academic_yr", academic_yr);
        HomeworkViewedByActivity.this.finish();
    }
